...
.. note::
   ...
   You can swap models or providers by changing ``BaseLLM`` / ``A2ALLM`` parameters. See :doc:`../components/agent_system` for interfaces.
...
.. tip:: Customization Options
   ...
   - ``rule_based``: set to ``True`` to use deterministic navigation; see :doc:`../components/agent_system`.
...
.. note::
   ...
   For action space details and planner behavior, read :doc:`../components/agent_system`, :doc:`../components/traffic_system`, and :doc:`../components/ue_detail`.
...
- Incorporate image observations via ``UnrealCV.get_image`` for VLM-based agents; see :doc:`../components/ue_detail` for view modes and camera controls.