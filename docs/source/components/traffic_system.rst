...
The diagram above illustrates the internal architecture of the Traffic System. The corresponding Python implementation can be found in ``simworld/traffic``. The system is initialized using a city layout generated by the :doc:`Procedural City Generation <../components/citygen>` pipeline. Based on this layout, traffic elements—including vehicles, pedestrians, and traffic signals—are sampled and registered into the simulation.
...
Next, establish a connection to Unreal Engine (UE) and spawn all necessary elements within the UE environment. For detailed usage, refer to the :doc:`UnrealCV+ <unrealcv+>`.
